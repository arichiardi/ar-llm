...

;;;; DB helpers — query org-roam SQLite via emacsql

(defun org-roam-pi--db-strip-quotes (val)
  "Strip embedded double quotes from VAL (org-roam DB stores strings with literal quotes)."
  (if (stringp val)
      (replace-regexp-in-string "^\"\|\"$" "" val)
    val))

(defun org-roam-pi--db-available-p ()
  "Return non-nil if org-roam DB can be queried via emacsql."
  (ignore-errors
    (and (require 'org-roam-db nil t)
         (fboundp 'org-roam-db-query)
         (fboundp 'emacsql)
         t)))

(defun org-roam-pi--parse-properties (props-str)
  "Parse PROPS-STR (plist stored in DB) into an alist, stripping quotes."
  (if (not (stringp props-str))
      nil
    (let ((pairs '())
          (key nil)
          (tokens (split-string props-str "\\s+" t "[^ ]*")))
      (dolist (tok tokens)
        (setq tok (org-roam-pi--db-strip-quotes tok))
        (if key
            (progn
              (push (cons key tok) pairs)
              (setq key nil))
          (setq key tok)))
      (nreverse pairs))))

(defun org-roam-pi--db-node-list ()
  "Fetch all nodes from org-roam DB as a list of alists.
Returns nil if DB is unavailable."
  (unless (org-roam-pi--db-available-p)
    nil)
  (let* ((db (ignore-errors (org-roam-db)))
         (rows (if db
                   (org-roam-db-query [:select [id title file level todo priority properties]
                                      :from nodes])
                 nil)))
    (when rows
      (mapcar (lambda (row)
                (let ((id (nth 0 row))
                  ...
                  (list (cons 'id id)
                        (cons 'title (org-roam-pi--db-strip-quotes title))
                        (cons 'file (org-roam-pi--db-strip-quotes file))
                        (cons 'level level)
                        (cons 'todo (or (org-roam-pi--db-strip-quotes todo) ""))
                        (cons 'priority (or (org-roam-pi--db-strip-quotes priority) ""))
                        (cons 'tags nil)
                        (cons 'aliases nil)
                        (cons 'refs nil)
                        (cons 'properties (org-roam-pi--parse-properties properties)))))
              rows))))

(defun org-roam-pi--db-node-from-id (id)
  "Fetch a single node from DB by ID. Returns alist or nil."
  (unless (org-roam-pi--db-available-p)
    nil)
  (let* ((db (ignore-errors (org-roam-db)))
         (row (if db
                  (car (org-roam-db-query [:select [id title file level todo priority properties]
                                          :from nodes
                                          :where (= id $s1)]
                                         id))
                nil)))
...
(defun org-roam-pi--db-node-from-title (title)
  "Fetch a node from DB by TITLE or alias. Returns alist or nil."
  (unless (org-roam-pi--db-available-p)
    nil)
  (let* ((db (ignore-errors (org-roam-db)))
         (matches (if db
                      (append
                       (mapcar #'car (org-roam-db-query [:select [id] :from nodes :where (= title $s1)] title))
                       (mapcar #'car (org-roam-db-query [:select [node-id] :from aliases :where (= alias $s1)] title)))
                    nil)))
    (when-let* ((nid (car matches))
                (node (org-roam-pi--db-node-from-id nid)))
      node)))
...
(defun org-roam-pi--scan-nodes ()
  "Build node cache from org-roam DB, falling back to file scanning."
  (setq org-roam-pi--node-cache '())
  (if-let* ((db-nodes (org-roam-pi--db-node-list)))
      (progn
        (org-roam-pi--dbg (format "scan-nodes: loaded %d nodes from DB" (length db-nodes)))
        (setq org-roam-pi--node-cache db-nodes))
    (org-roam-pi--dbg "scan-nodes: DB unavailable, falling back to file scan")
...
(defun org-roam-pi--node-from-id (id)
  "Find a node by ID from the cache, falling back to direct DB query."
  (or (seq-find (lambda (n) (string= (cdr (assoc 'id n)) id))
                org-roam-pi--node-cache)
      (org-roam-pi--db-node-from-id id)))

(defun org-roam-pi--node-from-title (title)
  "Find a node by TITLE from the cache, falling back to direct DB query."
  (or (seq-find (lambda (n) (string= (cdr (assoc 'title n)) title))
                org-roam-pi--node-cache)
      (org-roam-pi--db-node-from-title title)))
...
Returns a JSON string."
  ...
  (let* ((max (or max-results 10))
         ...
         (db-available (org-roam-pi--db-available-p))
         (id-set (make-hash-table :test 'equal)))
    ;; Search DB by title and aliases if available
    (when db-available
      (dolist (nid (append
                    ...
                    (mapcar #'car (org-roam-db-query [:select [node-id] :from aliases
                                                      :where (like alias $s1)]
                                                     (concat "%" query "%")))))
        (puthash nid t id-set)))
    ;; Also search cached nodes for properties match (DB stores properties as opaque plist string)
    (dolist (node (org-roam-pi--node-list))
      (when (< (length results) max)
        (let* ((nid (org-roam-pi--node-id node))
               (title (org-roam-pi--node-title node))
               (props (org-roam-pi--node-properties node))
               (aliases (org-roam-pi--node-aliases node))
               (matched (or (gethash nid id-set)
...
Uses org-roam SQLite DB when available, falls back to file scanning.
Returns a JSON string."
  (org-roam-pi--bootstrap)
    ...
    (if (not id)
      ...
      (if (not node)
        ...
        (let* ((dir (or direction "both"))
               (outgoing '())
               (incoming '())
               (use-db (and (require 'org-roam-db nil t)
                            (fboundp 'org-roam-db-query))))
          (if use-db
              ;; DB path: query SQLite directly
              (let* ((db (ignore-errors (org-roam-db)))
                     (dest-ids '())
                     (src-ids '()))
                (when db
                  (when (or (string= dir "outgoing") (string= dir "both"))
                    (dolist (row (emacsql db [:select [dest] :from links :where (= source $s1)] id))
                      (push (car row) dest-ids)))
                  (when (or (string= dir "incoming") (string= dir "both"))
                    (dolist (row (emacsql db [:select [source] :from links :where (= dest $s1)] id))
                      (push (car row) src-ids))))
                (org-roam-pi--dbg (format "links from db: %d outgoing, %d incoming" (length dest-ids) (length src-ids)))
                (dolist (dest-id (seq-uniq (nreverse dest-ids)))
                  (let ((dest-node (org-roam-pi--node-from-id dest-id)))
                    (when dest-node
                      (push (org-roam-pi--node-to-map dest-node org-roam-pi-directory) outgoing))))
                (dolist (src-id (seq-uniq (nreverse src-ids)))
                  (let ((src-node (org-roam-pi--node-from-id src-id)))
                    (when src-node
                      (push (org-roam-pi--node-to-map src-node org-roam-pi-directory) incoming)))))
            ;; Fallback: file scanning cache
            (when (or (string= dir "outgoing") (string= dir "both"))
            ...
            (let* ((cur-id (cdr (assoc 'id current)))
              ...
              (when cur-node
                (push `((title . ,(org-roam-pi--node-title cur-node))
                        ...
                        (hop . ,cur-hop))
                      results))
                ;; Get neighbors from DB or cache
                (when (< cur-hop max-hops)
                  (let* ((use-db (and (require 'org-roam-db nil t)
                                      (fboundp 'org-roam-db-query)))
                         (out-neighbors '())
                         (in-neighbors '()))
                    (if use-db
                        (let ((db (ignore-errors (org-roam-db))))
                          (when db
                            (dolist (row (emacsql db [:select [dest] :from links :where (= source $s1)] cur-id))
                              (push (car row) out-neighbors))
                            (dolist (row (emacsql db [:select [source] :from links :where (= dest $s1)] cur-id))
                              (push (car row) in-neighbors))))
                      (progn
                        (setq out-neighbors (org-roam-pi--node-outgoing cur-id))
                        (setq in-neighbors (org-roam-pi--node-incoming cur-id))))
                    (let* ((neighbors (seq-uniq (append out-neighbors in-neighbors))))
      ...
      (let ((entry-node (org-roam-pi--node-from-title entry-title)))
        (when entry-node
          (let ((lines (list (format "## %s" entry-title))))
            ...
            ;; Connected nodes from DB or cache
            (let* ((entry-id (org-roam-pi--node-id entry-node))
              ...
              (if use-db
                  (let ((db (ignore-errors (org-roam-db))))
                    (when db
                      (dolist (row (emacsql db [:select [dest] :from links :where (= source $s1)] entry-id))
                        (push (car row) outgoing-ids))))
                (setq outgoing-ids (org-roam-pi--node-outgoing entry-id)))
              (dolist (dest-id (seq-uniq (nreverse outgoing-ids)))
                (let ((dest-node (org-roam-pi--node-from-id dest-id)))
                  (when dest-node
...
(defun org-roam-pi--build-recent-modifications ()
  "Build recently modified files section using DB or filesystem timestamps.
Returns a string of markdown text."
  (let* ((one-day-ago (- (float-time) (* 24 60 60)))
         (recent '())
         (lines '("## Recently Modified"))
         (db-available (org-roam-pi--db-available-p)))
    ;; Try DB first for mtime lookups
    (if db-available
        (dolist (row (org-roam-db-query [:select [file mtime] :from files]))
          (let ((mtime (float-time (nth 1 row))))
            (when (> mtime one-day-ago)
              (push `((file . ,(nth 0 row)) (mtime . ,mtime)) recent))))
      ;; Fallback to filesystem
      (dolist (f (org-roam-pi--all-org-files))
